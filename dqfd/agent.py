...
# ================================= Added ======================================
    ...
    def evaluate_policy(self, test_episodes=10, max_episode_length=np.inf):
        # Run with the model
        episode_rewards = []
        episode_steps = []
        for i in range(10):
            ...
            tmp_episode_step = 0
            while tmp_episode_step < max_episode_length:
                tmp_episode_step += 1
                obs, done = self._forward(obs, is_train=False, update_memory=False)
                ...
                if done:
                    episode_rewards.append(tmp_episode_reward)
                    episode_steps.append(tmp_episode_step)
                    break

        print("Evaluation results:")
        print(f"Rewards: {episode_rewards}")
        print(f"Average Reward: {np.mean(episode_rewards).item()}")
        print(f"Duration: {episode_steps}")
        print(f"Average Duration: {np.mean(episode_steps).item()}")

        return np.mean(episode_rewards).item(), np.mean(episode_steps).item()
                
    def _rollback(self):
        train_eval, _ = self.evaluate_policy(5)
        print(train_eval)
        print(self.last_train_eval)
        if train_eval > self.last_train_eval: # Improved
            print("Policy improved. Update the policy.")
            self.last_train_eval = train_eval
            self.last_saved_weights = self.model.state_dict()
        else: # Worse
            print("Policy not improved. Rollback the policy.")
            self.model.load_state_dict(self.last_train_saved_weights)
